...
fn handle_update_orderables(game: &mut Game, dt: f32) {
    ...
    let orderable_query = game.world.query_mut::<(&Transform, &mut Orderable, &Selectable)>();
    ...
    for (idx, (_entity, (transform, orderable, _selectable))) in game.world.query_mut::<(&Transform, &mut Orderable, &Selectable)>().into_iter().filter(|s| s.1.2.is_selected).enumerate() {
